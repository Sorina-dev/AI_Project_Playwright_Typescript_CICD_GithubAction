...
test.describe(' API Testing Suite @api', () => {
  ...
  test.describe('ReqRes API Tests - User Management & Authentication @api', () => {
    ...
    test('GET - Retrieve users with pagination', async ({ request }) => {
      ...
      // Accept both 200 (success) and 401 (API key required) due to ReqRes API changes
      ...
      if (response.status() === 200) {
        ...
        // When API key is required (401), validate the error response
        const errorData = await response.json();
        expect((errorData as any).error).toBeDefined();
        console.log(`API returned ${response.status()}: API key required for external access`);
      }
    });
